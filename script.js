...
function saveIncome(pessoa) {
  ...
  // Atualizar o Firestore sem sobrescrever o outro
  const docRef = db.collection("financas").doc("rendas");
  docRef.get().then(doc => {
      const data = doc.exists ? doc.data() : {};
      data[pessoa] = dados;
      data[`${pessoa}Total`] = total;
      data.totalCasal = (data.vitorTotal || 0) + (data.alineTotal || 0);
      docRef.set(data);
      document.getElementById("total-casal").textContent = data.totalCasal.toFixed(2);
  });
}
